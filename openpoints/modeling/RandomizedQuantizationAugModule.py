...
from .build import Modeling

@Modeling.register_module()
class RandomizedQuantizationAugModule(nn.Module):
    def __init__(self, region_num = 64, collapse_to_val = 'inside_random', spacing='random', transforms_like=False, p_random_apply_rand_quant = 1):
        """
        region_num: int;
        """
        super().__init__()
    ...
    def forward(self, data):
        ...
        x = data['x']
        if len(x.shape) == 3:
            x = x.transpose(1, 2).unsqueeze(3).contiguous()
        """
        x: (B, c, H, W) or (C, H, W)
        """
        EPSILON = 1
        ...
        data['pose'] = x.squeeze(3).transpose(1, 2).contiguous()
        return data